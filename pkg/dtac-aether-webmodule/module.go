...
func NewAetherWebModule() *AetherWebModule {
	...
	awm.SetStaticFilesGetter(func() fs.FS {
		return awm.GetStaticFiles()
	})

	// Set default configuration with debug enabled
	awm.SetConfig(modules.WebModuleConfig{
		Port:        8090,
		StaticPath:  "/",
...
// AetherWebModule is the module struct that implements the Module interface
type AetherWebModule struct {
	// WebModuleBase provides web server functionality
	modules.WebModuleBase
}

// Name returns the name of the module type
func (h AetherWebModule) Name() string {
	t := reflect.TypeOf(h)
	return t.Name()
}

// GetStaticFiles returns the embedded filesystem for static assets
func (h *AetherWebModule) GetStaticFiles() fs.FS {
...
func (h *AetherWebModule) Register(request *api.ModuleRegisterRequest, reply *api.ModuleRegisterResponse) error {
